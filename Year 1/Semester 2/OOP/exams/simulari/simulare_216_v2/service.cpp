...
void Service::store(const string& name, const string& tara, const int puncte){
    Jucator jucator{name, tara, puncte, 0};
    this->repo.store(jucator);
    this->repo.recalc_ranking();
}

vector <Jucator> Service::filter(const string& tara) {
    auto all = this->repo.get_all();
    vector <Jucator> filtered;
