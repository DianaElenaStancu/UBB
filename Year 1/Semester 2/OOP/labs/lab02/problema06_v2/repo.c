...
bool AdaugaMaterieREPO(REPOSITORY_MATERII_P Repository, MATERIE_P Materie) {
    if (Repository == NULL || Materie == NULL) return 0;
...
bool ModificaMaterieREPO(REPOSITORY_MATERII_P Repository, MATERIE_P Materie) {
    ...
    SetProducator(Repository->Array[poz], GetProducator(*Materie));
    SetCantitate(Repository->Array[poz], GetCantitate(*Materie));
    return 1;
}
