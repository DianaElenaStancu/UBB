...
bool ModificaMaterieUi(UI_P Ui, char* erori) {
    ...
    scanf("%s %s %s", nume, producator, cantitate);
    cantitateINT = atoi(cantitate);
    success = ModificaMaterieSRV(Ui->Service, nume, producator, cantitateINT, erori);
    free(nume);
    free(producator);
    if (!success)
