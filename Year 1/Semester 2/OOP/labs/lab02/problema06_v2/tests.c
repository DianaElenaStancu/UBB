...
void testUtils() {
    ...
    SelectionSort(Array, ComparaCrescatorNume, 2);
    assert(MateriiEgale(Array[1], *m1));
    assert(MateriiEgale(Array[0], *m2));

    SelectionSort(Array, ComparaDescrescatorNume, 2);
    assert(MateriiEgale(Array[0], *m1));
    assert(MateriiEgale(Array[1], *m2));

    SelectionSort(Array, ComparaDescrescatorCantitate, 2);
    assert(MateriiEgale(Array[0], *m1));
    assert(MateriiEgale(Array[1], *m2));

    SelectionSort(Array, ComparaCrescatorCantitate, 2);
    assert(MateriiEgale(Array[1], *m1));
    assert(MateriiEgale(Array[0], *m2));
...
void testService() {
    ...
    assert(GetAllSRV(service, 2, "c", &Array, &len, erori) == 0);
    assert(strcmp(erori, "Nu se pot afisa materii prime cu filtrul dat!\n") == 0);
    free(Array);

    MATERIE_P Array1;
    assert(GetAllSRV(service, 1, "lapte", &Array1, &len, erori));
    assert(strlen(erori) == 0);
    assert(strcmp(GetNume(Array1[0]), "lapte") == 0);
    assert(strcmp(GetProducator(Array1[0]), "zuzu") == 0);
    assert(GetCantitate(Array1[0]) == 200);
    free(Array1);

    //MATERIE_P Array2;
    //assert(GetAllSRV(service, 2, "sahd", &Array2, &len, erori) == 0);
    //assert(strcmp(erori, "Nu se pot afisa materii prime cu filtrul dat!\n") == 0);
    //assert(GetAllSRV(service, 2, "10", &Array2, &len, erori) == 0);
    //assert(strcmp(erori, "Nu exista materii prime cu filtrul dat.\n") == 0);
    //free(Array2);
    MATERIE_P Array3;
    assert(GetAllSRV(service, 2, "300", &Array3, &len, erori));
    assert(strlen(erori) == 0);
    assert(strcmp(GetNume(Array3[0]), "lapte") == 0);
    assert(strcmp(GetProducator(Array3[0]), "zuzu") == 0);
    assert(GetCantitate(Array3[0]) == 200);
    free(Array3);
