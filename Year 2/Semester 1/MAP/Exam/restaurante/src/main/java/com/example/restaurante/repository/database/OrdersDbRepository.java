...
public class OrdersDbRepository extends ConcreteObservable<OrderEvent> implements Repository<Integer, Order> {
    ...
    public Order find(Integer id) {
        ...
        String sql = "SELECT o.id as order_id, o.status as status, o.table_id AS table_id, o.date AS order_date, " +
                "mi.id as item_id, mi.category as item_category, mi.item as item_item, mi.price as item_price, " +
                "mi.currency as item_currency, oi.quantity as item_quantity FROM orders o INNER JOIN order_items oi " +
                "ON oi.order_id = o.id INNER JOIN menu_items mi ON mi.id = oi.item_id WHERE o.id = ?";
