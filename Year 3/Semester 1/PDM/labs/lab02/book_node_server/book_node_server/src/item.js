 ...
 */
    async insert(item) {
        ...
        if(!item.pageCount) {
            throw new Error('Missing page count property')
        }
        if(!item.dateAdded) {
            throw new Error('Missing date added property')
        }
        if(!item.alreadyRead) {
            throw new Error('Missing already read property')
        }
...
itemRouter.get('/filter/:alreadyRead', async (ctx) => {
    const userId = ctx.state.user._id;
    const item = await itemStore.find({ alreadyRead: ctx.params.alreadyRead, userId });
    if (item) {
        if (item.userId === userId) {
            ctx.response.body = item;
            ctx.response.status = 200; // ok
