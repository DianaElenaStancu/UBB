    ...
    (state, { type, payload }) => {
        switch(type) {
            ...
            case FETCH_BOOKS_SUCCEEDED:
                return { ...state, books: payload.items, fetching: false };
...
export const BookProvider: React.FC<BookProviderProps> = ({ children }) => {
    ...
    const { books: books, fetching, fetchingError, saving, savingError } = state;
    useEffect(getBooksEffect, []);
    useEffect(wsEffect, []);
    const saveBook = useCallback<SaveBookFn>(saveBookCallback, []);
    const value = { books: books, fetching, fetchingError, saving, savingError, saveBook: saveBook };
    log('returns');
    return (
    ...
    function getBooksEffect() {
        ...
        async function fetchBooks() {
            try {
                ...
                if (!canceled) {
                    dispatch({ type: FETCH_BOOKS_SUCCEEDED, payload: { items: books } });
                }
            } catch (error) {
    ...
    function wsEffect() {
        ...
        const closeWebSocket = newWebSocket(message => {
            ...
            const { event, payload: { book: book }} = message;
